...
void _sym_push_path_constraint(nullable SymExpr constraint, int taken,
                               uintptr_t site_id, const char* filename = nullptr, uint32_t ln = 0, uint32_t col=0);
SymExpr _sym_get_input_byte(size_t offset, uint8_t concrete_value, const char* name = nullptr);
void _sym_make_symbolic(const void *data, size_t byte_length,
                        size_t input_offset, const char* name = nullptr);
 ...
 */
void symcc_make_symbolic(const void *start, size_t byte_length, const char* name = nullptr);
